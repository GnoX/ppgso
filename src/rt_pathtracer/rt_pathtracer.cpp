...
static void add_obj_to_scene(Scene *scene, std::string obj,
                             const Material &m,
                             glm::vec3 t = glm::vec3(1, -13, -3.0),
                             glm::vec3 r = glm::vec3(M_PI / 2, 0.0f, 0.0f),
                             glm::vec3 s = glm::vec3(15.0f)) {

    auto faces = load_obj(std::move(obj));
    glm::mat4 translation = glm::translate(glm::mat4(), t);
    glm::mat4 scale = glm::scale(glm::mat4(), s);
    glm::mat4 rotate = glm::orientate4(r);
...
//            }
//        }
        auto s0 = Sphere(1, glm::vec3(3, -12, 7), Material::RedMetal(.2f));
        scene->add(s0);
        auto s1 = Sphere(1, glm::vec3(3, -12, 3), Material::MetalScuffs());
        scene->add(s1);
        auto s2 = Sphere(1, glm::vec3(0, -12, 3), Material::Mirror());
        scene->add(s2);
        auto s3 = Sphere(1, glm::vec3(7, -12, 7), Material::Glass());
        scene->add(s3);
//        auto s4 = Sphere(3, glm::vec3(7, -5, 7), Material::Light());
//        scene->add(s4);
        add_obj_to_scene(scene.get(), "quad.obj", Material::Red());
        add_obj_to_scene(scene.get(), "quad.obj", Material::Light(), glm::vec3(1, -1, -3));
        add_obj_to_scene(scene.get(), "quad.obj", Material::Blue(), glm::vec3(1, -1, -3), glm::vec3(0));
        add_obj_to_scene(scene.get(), "quad.obj", Material::Red(), glm::vec3(1, -1, 12), glm::vec3(0));
        add_obj_to_scene(scene.get(), "quad.obj", Material::Green(), glm::vec3(12, -1, 3), glm::vec3(0, 0, M_PI / 2));
        add_obj_to_scene(scene.get(), "quad.obj", Material::Green(), glm::vec3(-5, -1, 3), glm::vec3(0, 0, M_PI / 2));
        add_obj_to_scene(scene.get(), "bottle.obj", Material::Glass(), glm::vec3(0, -12, 7), glm::vec3(-M_PI / 2, 0, 0), glm::vec3(.2f));
//        add_obj_to_scene(scene.get(), "bunny.obj", Material::RedMetal(), glm::vec3{3, -10, 3}, glm::vec3(0), glm::vec3(20.0f));
